...
import { MIDIReader } from "../midi/MIDIReader";
import { MIDIEventType } from "../midi/MIDIReader/MIDIEventType";
import { IMIDIEvent } from "../midi/MIDIReader/IMIDIEvent";
import { NoteNumberToName } from "../midi/utils/midi-note-converter";
import { NoteOnEvent } from "../midi/MIDIReader/MIDIEvents";
import { ITicksPerQuarterNote } from "../midi/MIDIReader/MIDIDivision";
import { TimeSignatureEvent, KeySignatureEvent } from "../midi/MIDIReader/MIDIEvents/Meta";
  ...
  for (let gen = reader.readTracks(), item = gen.next(); !item.done; item = gen.next()) {
    ...
    if (event.type & (MIDIEventType.NoteOn | MIDIEventType.NoteOff)) {
      ...
      if (type === MIDIEventType.NoteOn) {
        ...
        let { velocity } = event as NoteOnEvent;
...
export async function convertMIDI(
  ...
  if ((reader.header.division as ITicksPerQuarterNote).ticks) {
    xmlWriter.writeElement(
      "divisions",
      (reader.header.division as ITicksPerQuarterNote).ticks
    );
  }
